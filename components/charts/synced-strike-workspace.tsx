...
export function SyncedStrikeWorkspace({
  ...
  // 3. Compute GEX and Volume profiles by strike
  ...
  useEffect(() => {
    const allStrikes = Array.from(new Set([
      ...gexProfile.map(p => p.strike),
      ...volProfile.map(p => p.strike),
    ])).sort((a, b) => a - b)
  ...
  }, [gexProfile, volProfile, spotPrice, ticker, expiryMode])
  ...
  const handleMouseDown = (event: React.MouseEvent<SVGSVGElement>) => {
    ...
    // Y-axis lies on the right edge of the candlestick SVG (width - 45 to width)
    const isPriceScale = clickX >= (rect.width - 45)
  ...
  useEffect(() => {
    ...
    const margin = { top: 20, right: 45, bottom: 40, left: 15 }
    const chartHeight = dimensions.height - margin.top - margin.bottom

    const totalWidth = dimensions.width
    const candleWidth = totalWidth * 0.5
    const profileWidth = totalWidth * 0.25
    ...
    // ─── 2. GEX PROFILE CHART (25%) ───
    {
      ...
      const filteredGex = gexProfile.filter(p => p.strike >= yDomain[0] && p.strike <= yDomain[1])
      const maxGex = (d3.max(filteredGex.map(p => Math.abs(p.gex))) || 1) * 1.1
      ...
      // Draw solid filled GEX horizontal bars
      filteredGex.forEach(p => {
      ...
      g.append('rect')
        ...
        .on('mousemove', (event) => {
          ...
          const closest = filteredGex.reduce((prev, curr) => {
    ...
    // ─── 3. VOLUME PROFILE CHART (25%) ───
    {
      const g = volSvg.append('g').attr('transform', `translate(${margin.right},${margin.top})`)
      const width = profileWidth - margin.right - margin.left

      const filteredVol = volProfile.filter(p => p.strike >= yDomain[0] && p.strike <= yDomain[1])
      const maxVol = (d3.max(filteredVol.map(p => p.volume)) || 1) * 1.1

      const xScale = d3.scaleLinear()
        .domain([0, maxVol])
        .range([0, width])

      // Draw solid filled volume bars
      filteredVol.forEach(p => {
        const y = yScale(p.strike)
        if (y === undefined || y < 0 || y > chartHeight) return

        const val = p.volume
        const barWidth = xScale(val)

        g.append('rect')
          .attr('x', 0).attr('y', y - 2)
          .attr('width', Math.max(1.5, barWidth))
          .attr('height', 5)
          .attr('fill', 'rgba(0, 200, 255, 0.55)')
          .attr('stroke', '#00C8FF')
          .attr('stroke-width', 0.6)
          .attr('rx', 1)
      })
      ...
      // X Axis
      const xAxis = d3.axisBottom(xScale).ticks(3).tickFormat(d => formatCompact(d as number))
      const xAxisG = g.append('g').attr('transform', `translate(0,${chartHeight})`).call(xAxis)
      xAxisG.selectAll('line').attr('stroke', 'none')
      xAxisG.selectAll('path').attr('stroke', '#1A1A1A')
      xAxisG.selectAll('text').attr('fill', '#949494').style('font-family', typography.fontSans).style('font-size', '9px').attr('dy', '10px')
      ...
      g.append('rect')
        ...
        .on('mousemove', (event) => {
          ...
          const closest = filteredVol.reduce((prev, curr) => {
            return Math.abs(curr.strike - price) < Math.abs(prev.strike - price) ? curr : prev
          }, filteredVol[0])
          ...
          if (closest && tooltipRef.current && containerRef.current) {
            ...
            tooltipRef.current.innerHTML = `
              <div style="font-family:${typography.fontSans};font-size:12px;color:${colors.text.primary};font-weight:600">Strike ${closest.strike.toFixed(0)}</div>
              <div style="font-family:${typography.fontMono};font-size:11px;color:${colors.accent.cyan};margin-top:2.5px">
                Volume: ${closest.volume.toLocaleString()} contracts
              </div>
            `
            tooltipRef.current.style.opacity = '1'
            tooltipRef.current.style.left = `${event.clientX - containerRect.left + 14}px`
  ...
  }, [dimensions, yDomain, visibleCandlesData, indicatorData, gexProfile, volProfile, spotPrice, zeroGamma, market, ticker])
  ...
  return (
    ...
    >
      ...
      {/* GEX Profile SVG */}
      <svg ref={gexSvgRef} className="h-full" />

      {/* Volume Profile SVG */}
      <svg ref={volSvgRef} className="h-full" />
